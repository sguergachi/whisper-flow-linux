...
if scenario == "rows_exist":
    ...
    # Rebuild applies radio state via the main loop; without a turn, get_active
    # can still read the previous widget on a slow Windows runner.
    assert pump(lambda: (
        "ggml-small.en-q8_0" in w._model_checks
        and w._model_checks["ggml-small.en-q8_0"].get_active()
    ...
    subtitle = w._status_row.get_subtitle() or ""
    assert "4242" in subtitle, f"pid missing from status: {subtitle!r}"
...
def _run(tmp_path, scenario: str) -> subprocess.CompletedProcess:
    ...
    # stderr when the runner swallowed the traceback; surface returncode too.
    if "OK" not in result.stdout and not result.stderr:
        result.stderr = (
            f"(child exit {result.returncode}; no stderr captured)\n"
            f"stdout was:\n{result.stdout}"
        )
    return result
